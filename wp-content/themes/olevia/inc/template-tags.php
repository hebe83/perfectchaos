...
function olevia_print_the_content() {
  ...
  if ( is_home() || is_archive() || is_search() || is_page_template( 'page-templates/home.php' ) ) {
    if ( strpos( $post->post_content, '<!--more-->' ) ) {
      the_content( '' );
      echo '<div class="c-post__read-more"><a href="' . esc_url( get_permalink() ) . '">' . __( 'Read More', 'olevia' ) . '</a></div>';
    }
  ...
  <nav class="c-post-nav" role="navigation">
    ...
    <div class="c-post-nav__links">
      ...
      <div class="c-post-nav__link c-post-nav__link--prev">
        <h5><?php previous_post_link( '%link', '<i class="fa fa-chevron-left"></i> %title' ); ?></h5>
      </div>
      <?php endif; ?>
      <?php if ( $next ) : ?>
      <div class="c-post-nav__link c-post-nav__link--next">
        <h5><?php next_post_link( '%link', '%title <i class="fa fa-chevron-right"></i>' ); ?></h5>
      </div>
      <?php endif; ?>
